...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const atan1 = Math.atan2(y2, x2);
  const atan2 = Math.atan2(y1, x1);
...
function roundToPowerOfTen(num, pow) {
  const multi = 10 ** pow;
  const base = Math.round(num / multi);
  return base * multi;
}
...
function getSumOfDigits(num) {
  return [...num.toString(10)].reduce((sum, char) => sum + +char, 0);
}
...
function getSumOfNumbers(...args) {
  return [...args].reduce((sum, n) => sum + n, 0).toPrecision(1);
}
